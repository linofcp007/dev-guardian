...
export interface ProcessRunOptions {
  ...
  /**
   * Override the stdout buffer cap. Defaults to 5 MB. Set lower for
   * untrusted output or higher when a scanner is known to print
...
function attachStderr(child: ResultPromise, onChunk: (text: string) => void): void {
  child.stderr?.on('data', (chunk: Buffer) => {
    onChunk(chunk.toString('utf8'));
  });
}

function safeKill(child: ResultPromise): void {
  if (!child.pid) return;
