...
export type SbomFormat = 'cyclonedx-json' | 'spdx-json' | 'unknown';
...
export interface SbomSummary {
  ...
  top_packages: SbomComponent[];
  inline_size_bytes: number;
}

export function detectFormat(input: unknown): SbomFormat {
  ...
  if (bomFormat && bomFormat.toLowerCase() === 'cyclonedx') return 'cyclonedx-json';
  if (getString(root, 'specVersion') && getProp(root, 'components')) return 'cyclonedx-json';
  // SPDX: SPDXID + spdxVersion present.
  if (getString(root, 'SPDXID') || getString(root, 'spdxVersion')) return 'spdx-json';
  return 'unknown';
}

export function summarize(input: unknown, topN = 25): SbomSummary {
...
function extractSpdx(root: unknown): SbomComponent[] {
  const out: SbomComponent[] = [];
