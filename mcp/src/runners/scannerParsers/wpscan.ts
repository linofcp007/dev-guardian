...
export const wpscanParser: ScannerParser = {
  ...
  parse(input: unknown, _ctx: ParserContext = {}): ParserOutput {
    ...
    if (plugins && typeof plugins === 'object') {
      for (const [slug, raw] of Object.entries(plugins as Record<string, unknown>)) {
    ...
    if (themes && typeof themes === 'object') {
      for (const [slug, raw] of Object.entries(themes as Record<string, unknown>)) {
        const ver = getString(getProp(raw, 'version'), 'number') ?? 'unknown';
        for (const v of asArray(getProp(raw, 'vulnerabilities'))) {
          pushVuln(v, 'wordpress-theme', `${slug}@${ver}`, findings, cves);
        }
...
function pushVuln(
  raw: unknown,
  subcategory: 'wordpress-core' | 'wordpress-plugin' | 'wordpress-theme',
  ...
  for (const cveId of cveList) {
    ...
    const cveEntry: ParserCveInput = {
      cve_id: cveId,
      package_name: pkgName ?? componentLabel,
      severity,
    };
    if (installedVersion !== undefined) cveEntry.installed_version = installedVersion;
    if (fixedIn !== undefined) cveEntry.fixed_version = fixedIn;
    cves.push(cveEntry);
  }
}

function severityFromVuln(raw: unknown): Severity {
