...
export function importSpec(file: string, text: string): SpecImportResult {
  ...
  const { base, partial: basePartial } =
    format === 'openapi-3' ? openapiBasePath(root) : swaggerBasePath(root);
...
/* ---- parsing: JSON first, YAML (with line info) on failure -------------- */

type ParsedRoot =
  | { kind: 'ok'; root: unknown; lineFor: (pathKey: string) => number }
...
function detectFormat(root: unknown): SpecFileReport['format'] {
  ...
  if (openapi !== undefined && openapi.startsWith('3.')) return 'openapi-3';
  if (str(root, 'swagger') === '2.0') return 'swagger-2';
  return 'unknown';
}

function openapiBasePath(root: unknown): { base: string; partial: boolean } {
  const servers = asArray(prop(root, 'servers'));
  const url = str(servers[0], 'url');
  if (url === undefined) return { base: '', partial: false };
 ...
 */
function authHint(
  rawOperationSecurity: unknown,
  docSecurityNonEmpty: boolean,
): RouteRecord['auth_hint'] {
...
function paramsFromTemplate(pathTemplate: string): string[] {
  const names: string[] = [];
