...
const ACTIX_VERBS = ['get', 'post', 'put', 'patch', 'delete', 'options', 'head'] as const;

/** `metadata.method` is interpolated into an attribute name; keep it a word. */
const PLAIN_VERB = /^[A-Za-z]+$/;

interface AttributeName {
  /** Literal text to look for, e.g. `HttpGet`, `Get`, `get`. */
  text: string;
  /** The HTTP verb, when the attribute name IS the verb (Rust). */
  verb?: string;
}
...
function routeAttributeNames(
  ...
  if (method === undefined || !PLAIN_VERB.test(method)) return undefined;
  const name = method.charAt(0).toUpperCase() + method.slice(1).toLowerCase();
  if (framework === 'nestjs') return [{ text: name }]; // `@Get(`
  if (framework === 'aspnet') return [{ text: `Http${name}` }]; // `[HttpGet(`
  return undefined;
}
 ...
 */
function isAttributePosition(span: string, framework: string, at: number): boolean {
...
function lastNonSpaceBefore(span: string, index: number): number {
  let i = index - 1;
  while (i >= 0) {
    const ch = span[i];
...
function isWordBoundary(span: string, at: number, length: number): boolean {
 ...
 */
function findRouteAttribute(
  span: string,
  framework: string,
  names: AttributeName[],
): { open: number; verb: string | undefined } | undefined {
  for (let i = 0; i < span.length; i += 1) {
    for (const name of names) {
      if (!span.startsWith(name.text, i)) continue;
      if (!isWordBoundary(span, i, name.text.length)) continue;

      let open = i + name.text.length;
      while (open < span.length && /\s/.test(span[open] ?? '')) open += 1;
      if (span[open] !== '(') continue;

      if (!isAttributePosition(span, framework, i)) continue;
      if (matchingClose(span, open) === undefined) continue;
