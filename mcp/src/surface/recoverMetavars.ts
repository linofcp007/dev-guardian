...
function routePath(span: string): string | undefined {
  ...
  if (open !== undefined) return argumentsAt(span, open)?.[0];
 ...
 */
function callOpener(span: string): number | undefined {
 ...
 */
function outermostCallOpener(span: string): number | undefined {
...
function lastNonSpaceIndex(span: string): number | undefined {
...
function synthesizeMount(span: string): Metavars | undefined {
  ...
  if (prefix === undefined || last === undefined) return undefined;
 ...
 * guessed at.
 */
function joinPythonModule(module: string): string {
...
function synthesizeRustImport(span: string): Metavars | undefined {
  ...
  if (braceIdx !== -1) {
    ...
    if (prefix.length === 0 || item === undefined || item.length === 0) return undefined;
  ...
  if (split === undefined || split.module.length === 0) return undefined;
  return {
 ...
 * → $SYMBOL="get") forms.
 */
function stripRustAlias(path: string): string {
  return path.replace(/\s+as\s+[A-Za-z_]\w*$/, '');
}

/** Split at the LAST `::`; a path with none is all symbol and no module. */
function splitRustPath(path: string): { module: string; symbol: string } | undefined {
  const lastSep = path.lastIndexOf('::');
  if (lastSep === -1) return path.length === 0 ? undefined : { module: '', symbol: path };
...
function synthesizePhpImport(span: string): Metavars | undefined {
  ...
  if (braceIdx !== -1) {
    ...
    if (prefix.length === 0 || symbol === undefined || symbol.length === 0) return undefined;
    return {
...
function synthesizeEnv(span: string): Metavars | undefined {
  ...
  if (destructured !== undefined) return { $NAME: { abstract_content: destructured } };
