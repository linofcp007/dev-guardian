...
export interface ImportGraph {
  ...
  /**
   * Every file that appears as an importer or an import target, including
   * one whose own edge was cut by the cap. A file present here but with no
   * path from any root is "seen and unreached"; a file absent from it was
   * never named by any import record. A later stage tells those two facts
   * apart by testing this set — conflating them would turn "we never looked"
   * into "we looked and found nothing".
   */
...
export function buildImportGraph(records: readonly { file: string; module_file: string }[]): ImportGraph {
  ...
  for (const record of records) {
    ...
    if (existing !== undefined && existing.has(record.module_file)) continue;
 ...
 */
function shortestDistance(graph: ImportGraph, root: string, target: string): number | null {
...
export function reachFrom(graph: ImportGraph, roots: readonly string[], target: string): ReachResult {
  const found: Array<{ root: string; hops: number }> = [];
