...
export function renderDashboard(snapshot: DashboardSnapshot): string {
  ...
  if (banner !== null) sections.push(banner);
 ...
 */
function renderNoScan(snapshot: DashboardSnapshot): string {
  const sections = [
    `<section>
...
function riskSection(snapshot: DashboardSnapshot): string {
  const { risk } = snapshot;
...
function truncationSection(notices: readonly TruncationNotice[]): string | null {
  if (notices.length === 0) return null;
  const items = notices
    .map(
      (n) =>
        `<li><code>${escapeHtml(n.what)}</code>: showing ${n.shown} of ${n.total} (${escapeHtml(n.reason)})</li>`,
    )
    .join('');
  return `<section>
  <h2>What was cut</h2>
...
function severitySection(findings: FindingsSummary): string {
...
function deltasSection(snapshot: DashboardSnapshot, scanType: string): string {
  const previous = renderDelta(
    'Since last scan',
    snapshot.deltas.since_previous,
    `no previous ${scanType} scan to compare`,
  );
...
function renderDelta(label: string, delta: FindingDelta | null, emptyMessage: string): string {
  if (delta === null) {
...
function deltaRow(f: Finding): string {
...
function findingsTableSection(findings: FindingsSummary): string {
...
function optionsFor(counts: Record<string, number>): string {
...
function cvesSection(cves: CveSummary): string {
...
function suppressionsSection(s: SuppressionState): string {
  if (s.active_count === 0) {
    return `<section>
  <h2>Suppressions</h2>
...
function scanMetaSection(snapshot: DashboardSnapshot): string {
  const scan = snapshot.scan;
  if (scan === null) return '';
...
function baselineMetaLine(baseline: BaselineState): string {
  if (baseline.active === null) return '';
 ...
 */
function payloadScript(snapshot: DashboardSnapshot): string {
