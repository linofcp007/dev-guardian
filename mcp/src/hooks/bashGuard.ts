 ...
 */
function assessRmCommand(cmd: string): MatchedRule | null {
  const segments = cmd.split(/&&|\|\||[;|&]/);
  let broad: MatchedRule | null = null;

  for (const seg of segments) {
    const tokens = seg.trim().split(/\s+/).filter(Boolean);
    let i = 0;
    while (i < tokens.length && (tokens[i] === 'sudo' || tokens[i] === 'command')) i++;
    ...
    if (!((recursive && force) || noPreserve)) continue;
...
export function assessBashCommand(command: string): BashAssessment {
  ...
  for (const rule of BASH_RULES) {
    if (rule.pattern.test(cmd)) matched.push({ id: rule.id, level: rule.level, reason: rule.reason });
  }
  const rm = assessRmCommand(cmd);
  if (rm) matched.push(rm);
