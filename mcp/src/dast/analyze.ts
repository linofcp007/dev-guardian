 ...
 */
export function dastRuleId(check: DastCheck, method: string, path: string): string {
...
interface BuildFindingArgs {
  ...
  /**
   * Present only for a finding whose identity is the origin itself, not any
   * one route or request: routes `fingerprint`/`rule_id` through
   * `originFingerprint`/`originRuleId` instead of the (method, path) pair
   * otherwise derived from `request`. `evidence_id` is unaffected by this —
   * it always names the real request that happened to produce this finding
   * instance, which is deliberately allowed to float even when the identity
 ...
 */
function isImpliedMethod(method: string, known: ReadonlySet<string>): boolean {
