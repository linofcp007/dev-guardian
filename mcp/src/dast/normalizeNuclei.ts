...
import { dastFingerprint, type DastFinding } from './analyze.js';
 ...
 */
function pathnameOf(matchedAt: string): string {
  if (matchedAt === '') return '';
  try {
 ...
 */
function matchRoute(pathname: string, routes: readonly RouteRecord[]): RouteRecord | undefined {
...
function buildMessage(templateId: string, matchedAt: string, description: string | undefined): string {
...
function normalizeLine(line: string, routes: readonly RouteRecord[]): DastFinding | null {
  let parsed: unknown;
  try {
 ...
 */
export function normalizeNucleiJsonl(
  jsonl: string,
