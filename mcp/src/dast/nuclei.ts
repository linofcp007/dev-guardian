...
export interface NucleiRunOptions {
  ...
  targetUrl: string;
  outputPath: string;
  /** False (the default envelope) excludes `intrusive`-tagged templates too; see `buildNucleiArgs`. */
 ...
 */
const ALWAYS_EXCLUDED_TAGS = ['dos', 'fuzz'] as const;

function excludedTags(allowIntrusive: boolean): string[] {
 ...
 */
export function interpretRun(run: ProcessRunResult): { ok: boolean; reason?: string } {
