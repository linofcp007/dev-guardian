...
registerToolModule(
  makeScanTool({
    ...
    description:
      'Run the full open-source security toolchain (Semgrep, gitleaks, Trivy, Bandit when applicable). ' +
    ...
    invoke: async (_input, ctx): Promise<ScannerInvocation> => {
      ...
      const tools_run: ScannerInvocation['tools_run'] = [];
      ...
      const result: ScannerInvocation = {
