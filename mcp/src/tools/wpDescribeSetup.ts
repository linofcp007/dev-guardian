...
const tool: ToolModule = {
  ...
  description:
    'Aggregate read of accumulated WP state: latest wp_audit (versions, checksum mismatches, ' +
...
async function handler(ctx: PluginContext): Promise<ToolResult<Record<string, unknown>>> {
  ...
  return {
    ...
    audits: {
      wp_audit: wpAudit
        ? {
            ...
            wp_version: (wpAudit.meta as { wp_version?: string } | undefined)?.wp_version ?? null,
            admins_count: ((wpAudit.meta as { admins?: unknown[] } | undefined)?.admins ?? [])
              .length,
            checksum_mismatches_count: countChecksumIssues(wpAudit.meta),
            warnings: (wpAudit.meta as { warnings?: string[] } | undefined)?.warnings ?? [],
          }
        : null,
      wp_cron_audit: wpCron
        ? {
            ...
            flagged_count:
              (wpCron.meta as { flagged_count?: number } | undefined)?.flagged_count ?? 0,
          }
        : null,
      wp_rest_audit: wpRest
        ? {
            ...
            exposed_count:
              (wpRest.meta as { exposed_count?: number } | undefined)?.exposed_count ?? 0,
          }
        : null,
    ...
    active_cves: cves,
    recommended_next:
      !wpAudit ? 'Run `wp_audit` first to capture baseline state.'
...
function countChecksumIssues(meta: unknown): number {
  const cm = (meta as { checksum_mismatches?: {
    core?: unknown[];
    plugins?: Record<string, unknown[]>;
    themes?: Record<string, unknown[]>;
  }} | undefined)?.checksum_mismatches ?? {};
  return (
    (cm.core?.length ?? 0) +
