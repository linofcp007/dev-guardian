...
interface FrameworkSupport {
  tfm: string;
  status: 'lts-current' | 'sts-current' | 'eol' | 'legacy' | 'unknown';
  hint: string;
}

const SUPPORT: Record<string, FrameworkSupport> = {
...
const tool: ToolModule = {
  ...
  description:
    'Parse every *.csproj / *.fsproj `<TargetFramework>` and report support status (LTS, STS, EOL, legacy ' +
...
function extractTfms(xml: string): string[] {
  ...
  for (const m of singleMatches) {
    ...
    if (inner && inner[1]) out.push(inner[1].trim());
  }
  ...
  for (const m of multiMatches) {
    ...
    if (inner && inner[1]) {
      for (const tfm of inner[1].split(';')) {
        if (tfm.trim().length > 0) out.push(tfm.trim());
      }
    }
  }
  return out;
...
function unknownStatus(tfm: string): FrameworkSupport {
  return {
    tfm,
    status: 'unknown',
...
function collectCsprojFiles(root: string, maxDepth: number): string[] {
  const out: string[] = [];
