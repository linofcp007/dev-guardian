...
const inputSchema = {
  ...
  scope: z
    ...
    .optional()
    .describe(
      'Where to register the MCP server: "project" (config inside the repo, default) or "global" ' +
...
interface HostResult extends RulesResult {
  host: HostName;
  scope: McpScope;
  mcp: McpResult;
}
...
const tool: ToolModule = {
  ...
  description:
    'Make a non-Claude AI host dev-guardian-aware: register the MCP server in its config (merging, ' +
...
async function handler(
  ...
  const inp = input as {
    ...
    host: HostArg;
    scope?: McpScope;
    register_mcp?: boolean;
    install_rules?: boolean;
    force?: boolean;
    apply?: boolean;
  };
  ...
  const requestedScope: McpScope = inp.scope ?? 'project';

  const requestedHosts: HostName[] = inp.host === 'all' ? [...ALL_HOSTS] : [inp.host];
  ...
  return {
    ...
    results,
    next_steps:
      'Restart the target AI host so it re-reads its MCP config and rules file. Then ask "what ' +
