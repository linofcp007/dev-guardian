...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    severity: 'high',
    test: (l) =>
      /migrationBuilder\.AlterColumn/.test(l) &&
  ...
  },
  {
    ...
    severity: 'critical',
    test: (l) =>
      /migrationBuilder\.Sql\s*\(/.test(l) && /(password|secret|api[_-]?key|token)/i.test(l),
  },
];
...
const tool: ToolModule = {
  ...
  description:
    'Scan EF Core migrations for dangerous patterns (DropTable, DropColumn, AlterColumn nullable=false ' +
...
function findMigrationsDirs(root: string): string[] {
  const out: string[] = [];
