...
import type { DriftEntry, DriftReport } from './detect.js';
 ...
 */
export function buildDriftAdvisory(report: DriftReport): string | null {
  ...
  if (upstream.length > 0) {
    clauses.push(
      `${list(upstream.map(installedBy))} ${upstream.length === 1 ? 'is' : 'are'} unchanged since ` +
  ...
  if (diverged.length > 0) {
    clauses.push(
      `${list(diverged.map((e) => e.target))} changed on both sides since install, so bringing ` +
  ...
  if (pending.length > 0) {
    clauses.push(
      `a newer baseline is already waiting at ${list(pending.map(deliveredPath))} — merge it into ` +
  ...
  if (upstream.length > 0 || diverged.length > 0) {
    clauses.push(
      'Run init_project with refresh=true and apply=false to preview, then apply=true to update; ' +
...
function installedBy(entry: DriftEntry): string {
  return `${entry.target} (installed by plugin v${entry.recorded_plugin_version})`;
}

function deliveredPath(entry: DriftEntry): string {
 ...
 */
function currentVersion(entries: DriftEntry[]): string {
