 ...
 * edit here.
 */
export function commentPrefixFor(targetPath: string): string | null {
...
export interface ProvenanceHeaderInput {
  /** Path relative to `configs/`, e.g. `semgrep/base.yml`. */
  source: string;
  /** Plugin version at copy time, from `.claude-plugin/plugin.json`. */
  pluginVersion: string;
  /** Line-comment prefix, from `commentPrefixFor`. */
  prefix: string;
}
 ...
 */
export function buildProvenanceHeader(input: ProvenanceHeaderInput): string {
...
export function stripProvenanceHeader(text: string): string {
  ...
  const first = lines[0];
  if (first === undefined || !first.includes(PROVENANCE_MARKER)) return text;
  ...
  for (let i = 0; i < limit; i += 1) {
    ...
    let start = i + 1;
    if (lines[start] === '') start += 1; // the blank separator we emit
