...
export type DriftState =
  /** Both sides match what was recorded. Silent. */
  | 'in_sync'
  /** Their hash moved, ours did not. Expected and fine. Silent. */
  | 'local_edit'
  /** Ours moved, theirs did not. The state this whole mechanism exists for. */
  | 'upstream_update'
  /** Both moved. A refresh will need a merge. */
  | 'diverged'
  /** A refresh already delivered an update as `<target>.new`, still on disk. */
  | 'pending_merge'
  /** The user deleted their copy. Their call. Silent. */
  | 'target_missing'
...
export interface DriftEntry {
  ...
  source: string;
  state: DriftState;
  recorded_plugin_version: string;
  current_plugin_version: string;
  /** Set when `state` is `pending_merge`. */
  delivered_as?: string;
}
...
export interface DetectDriftInput {
  projectPath: string;
  /** Absolute path to the plugin's `configs/` directory. */
  configsDir: string;
  /** Version this build reports, from `platform/version.ts`. */
 ...
 */
export function detectConfigDrift(input: DetectDriftInput): DriftReport {
...
function classify(entry: ConfigManifestEntry, input: DetectDriftInput): DriftEntry {
  const base = {
    target: entry.target,
    source: entry.source,
