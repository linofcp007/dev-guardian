...
import type { FixGroup, FixSource } from './types.js';

/** Why one finding is not a fix candidate. Exactly one applies to each — see
 *  the module comment on why they are evaluated in this order. */
export type ExclusionReason =
  /** `fix_available === false`: no scanner offered a fix. No floor recovers it. */
  | 'no_fix_available'
...
export interface FixExclusions {
  /** Every open finding the run started from. */
  considered: number;
  /** Findings covered by at least one group, BEFORE `max_prs` defers any.
   *  The cap is reported separately, in `deferred`/`deferred_reason`. */
  candidates: number;
  /** `considered - candidates`. */
  excluded: number;
  /** Sums to `excluded`, always. */
  by_reason: Record<ExclusionReason, number>;
  /** Tiers and a suggested floor for the `below_severity_min` bucket alone. */
...
export function summariseExclusions(input: {
  ...
  const by_reason: Record<ExclusionReason, number> = {
    no_fix_available: 0,
...
export function describeExclusions(
  ...
  if (below_severity_min > 0) {
    parts.push(
      `${below_severity_min} below severity_min "${severityMin}" ` +
        `(${describeShortfallTiers(exclusions.below_severity_min)})`,
    );
  }
  if (no_fix_available > 0) {
    parts.push(
      `${no_fix_available} with no scanner-produced fix (fix_available=false — this tool only ` +
  ...
  const head =
    `${exclusions.excluded} of ${exclusions.considered} open finding(s) were excluded; ` +
...
function severityHint(shortfall: SeverityShortfall): string {
