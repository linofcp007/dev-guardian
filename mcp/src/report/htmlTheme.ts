...
const SHARED = { '--accent': '#00AAFF', '--brand-blue': '#11689B' } as const;
...
function chipColor(sev: string): { bg: string; fg: string } {
  return (SEVERITY_COLORS as Record<string, { bg: string; fg: string }>)[sev] ?? SEVERITY_COLORS.info;
}
...
export function severityChip(sev: string, label?: string): string {
...
export function severityBar(counts: Record<string, number>): string {
...
function cssVars(obj: Record<string, string>): string {
...
export interface HtmlDocument {
  title: string;
  subtitle?: string;
  /** Body blocks, already rendered to HTML. */
  sections: string[];
  /** Report language for the shell chrome (footer, <html lang>). Default 'en'. */
  lang?: Lang;
}

/** Wrap pre-rendered section HTML in the branded, self-contained shell. */
export function renderHtmlDocument(doc: HtmlDocument): string {
...
function sanitizeUrl(u: string): string | null {
  ...
  if (/^[\w./#?=&%-]+$/.test(t)) return t; // bare relative path
  return null;
}
...
function inline(raw: string): string {
  ...
  s = s.replace(/\[\[\[CODE_(\d+)\]\]\]/g, (_m, i: string) => codes[Number(i)] ?? '');
  return s;
}
