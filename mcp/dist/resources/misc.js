...
registerResourceModule({
    name: 'guardian-sbom',
    uri: 'guardian://sbom',
    description: 'Metadata for the most recent SBOM produced by `generate_sbom`: format, produced_by, file ' +
...
registerResourceModule({
    name: 'guardian-stack',
    uri: 'guardian://stack',
    description: 'Latest stack snapshot produced by `detect_stack`. Returns `{ snapshot: null }` when no ' +
        'snapshot exists yet.',
    handler: async (_uri, _params, ctx) => {
...
registerResourceModule({
    ...
    description: 'Compliance status from the most recent compliance_check: licenses_summary, risky_licenses, ' +
...
registerResourceModule({
    ...
    description: 'Active regression baseline: `{ baseline_id, scan_id, set_at, note? }`. Returns ' +
...
function findLatestOfType(ctx, acceptedTypes) {
    const history = ctx.storage.scans.listHistory(50);
    const found = history.find((s) => s.status === 'completed' && acceptedTypes.includes(s.scan_type));
