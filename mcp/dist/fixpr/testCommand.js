...
export const TEST_MANIFESTS = CHECKERS.map(([name]) => name);
export function deriveTestCommand(files) {
...
function fromPackageJson(content) {
    ...
    if (trimmed.length === 0 || trimmed === NPM_PLACEHOLDER_TEST_SCRIPT)
        return null;
...
function fromPyprojectToml(content) {
    if (!PYTEST_SECTION.test(content))
        return null;
