...
const DEP_SCANNER_TOOLS = ['trivy', 'npm-audit', 'wpscan'];
export function buildGroups(input) {
    ...
    const eligible = input.findings.filter((finding) => finding.fix_available && passes(finding.severity, input.severityMin));
    const groups = [];
...
function buildDepsGroups(findings, upgradeSteps) {
    ...
    for (const bucket of buckets.values()) {
        ...
        if (list === undefined)
            byEcosystem.set(bucket.step.ecosystem, [candidate]);
        else
            list.push(candidate);
    }
    return [...byEcosystem.entries()].map(([ecosystem, candidates]) => makeGroup('deps', ecosystem, candidates));
}
function mentionsPackage(finding, packageName) {
    return (containsWholePackageName(finding.title, packageName) ||
...
function containsWholePackageName(text, packageName) {
    ...
    for (;;) {
        ...
        if (!isPackageNameChar(before) && !isPackageNameChar(after))
            return true;
        from = index + 1;
    }
}
