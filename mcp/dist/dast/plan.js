    ...
    for (const r of routes) {
        ...
        for (const method of fresh) {
            ...
            if (opts.authHeaderValue !== null) {
                requests.push(build(method, path, 'authenticated', { authorization: opts.authHeaderValue }, opts, synthetic, routeIndex));
            }
        }
        if (needCors) {
            seen.add(corsKey);
            requests.push(build('GET', path, 'cors', { origin: CORS_PROBE_ORIGIN }, opts, synthetic, routeIndex));
        }
    }
...
function build(method, path, variant, extraHeaders, opts, synthetic, routeIndex) {
    const isWrite = WRITE_METHODS.includes(method);
    const req = {
