 ...
 */
function originRuleId(check, origin, parts = []) {
 ...
 */
export function originFingerprint(check, origin, parts = []) {
...
function checkMethodSurface(input, findings) {
    ...
    for (const r of input.results) {
        ...
        const extra = [...new Set(allow.split(',').map((m) => m.trim().toUpperCase())
        ...
        if (banner !== null) {
            ...
            if (!reportedBanners.has(key)) {
                ...
                findings.push(buildFinding({
                    ...
                    message: `${input.origin} discloses ${banner.header}: ${banner.value} in its response ` +
