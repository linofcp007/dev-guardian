 ...
 */
export function livenessRequest(origin) {
    return {
 ...
 */
export function livenessMessage(target, liveness, timeoutMs) {
    const detail = liveness.outcome === 'timeout'
        ? `nothing answered within ${timeoutMs}ms`
        : `the connection failed (${liveness.error ?? 'no detail'})`;
    return (`Nothing is listening at ${target.origin} — ${detail}. Start the application first, then ` +
