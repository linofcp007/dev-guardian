...
const tool = {
    ...
    description: 'Propose stack-appropriate observability files (Pino logger / structlog / Monolog, plus a ' +
...
function inferStack(projectPath, ctx) {
    ...
    if (snap) {
        if (snap.languages?.includes('javascript') || snap.languages?.includes('typescript'))
            return 'node';
        if (snap.languages?.includes('python'))
            return 'python';
        if (snap.languages?.includes('php'))
            return 'php';
        if (snap.languages?.includes('go'))
            return 'go';
        if (snap.languages?.includes('rust'))
            return 'rust';
        if (snap.languages?.includes('java'))
            return 'java';
        if (snap.languages?.includes('ruby'))
            return 'ruby';
        if (snap.languages?.includes('csharp'))
            return 'dotnet';
    }
    // Filesystem fallback.
...
//# sourceMappingURL=observabilitySetup.js.map