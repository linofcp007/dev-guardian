...
const tool = {
    ...
    description: 'List WP scheduled cron events and flag suspicious ones: unknown hook namespaces, ' +
...
async function handler(input, ctx) {
    ...
    if (!wpBin) {
        return failDomain('missing_scanner', 'WP-CLI (`wp`) is required for wp_cron_audit. Run install_toolchain with tools=["wp-cli"].');
    }
    // Cron events
    ...
    if (cronResult.outcome !== 'completed') {
        return failDomain('scanner_failed', `wp cron event list failed: ${cronResult.stderr.split(/\r?\n/)[0] ?? cronResult.outcome}`);
    }
    let events = [];
    try {
        events = JSON.parse(cronResult.stdout);
    }
    catch {
        /* leave events empty */
    }
...
function matchesKnownPrefix(hook, activePlugins) {
    if (KNOWN_PREFIXES.some((p) => hook.startsWith(p)))
        return true;
    for (const plugin of activePlugins) {
        const slug = plugin.split('/')[0]?.replace(/\.php$/, '');
        if (slug && hook.toLowerCase().includes(slug.toLowerCase()))
            return true;
    }
    return false;
}
function matchesInactivePlugin(hook, inactivePlugins) {
...
//# sourceMappingURL=wpCronAudit.js.map