...
const tool = {
    ...
    description: 'Compare the active baseline (or previous completed scan) against the latest scan and flag ' +
...
async function handler(input, ctx) {
    ...
    if (!baselineId) {
        ...
        const prev = history.find((s) => s.scan_type === latest.scan_type && s.scan_id !== latest.scan_id && s.status === 'completed');
