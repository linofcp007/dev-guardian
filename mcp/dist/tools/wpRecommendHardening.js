...
const tool = {
    ...
    description: 'Generate a prioritised hardening checklist (Markdown) from the latest wp_audit. Pure read — ' +
...
async function handler(ctx) {
    ...
    const items = [];
    // Config flags
    const flags = (meta['config_flags'] ?? {});
    if (flags['DISALLOW_FILE_EDIT'] !== true) {
        items.push({
            priority: 'high',
            category: 'config',
            recommendation: "Set DISALLOW_FILE_EDIT=true in wp-config.php",
            rationale: 'Stops admins from editing PHP files via the WP dashboard. Standard hardening step — ' +
    ...
    if (flags['WP_DEBUG'] === true) {
        items.push({
            ...
            rationale: 'WP_DEBUG can leak stack traces, file paths, and DB errors to attackers. Should never be on in prod.',
        });
    }
    ...
    if (riskyAdmins.length > 0) {
        items.push({
            ...
            rationale: 'Standard credential-stuffing / brute-force attacks target the literal logins `admin`, ' +
    ...
    if (admins.length > 3) {
        items.push({
            ...
            rationale: 'Each admin is a credential that can be phished / stolen. Move ex-staff to Editor or remove.',
        });
    }
    // Checksum integrity
    const checksum = (meta['checksum_mismatches'] ?? {});
    if (checksum.core && checksum.core.length > 0) {
        items.push({
            ...
            rationale: 'Core files MUST match the published checksum. Differences = possible compromise. Reinstall ' +
    ...
    if (pluginMismatchCount > 0) {
        items.push({
            ...
            rationale: 'Modified plugin files are a backdoor vector. Re-install affected plugins from a clean source; ' +
                'inspect each modified file before deleting/replacing.',
        });
    }
    // Auto-update plugins
    const autoUpdate = (meta['plugins_with_auto_update'] ?? []);
    if (autoUpdate.length === 0) {
        items.push({
            priority: 'medium',
            category: 'plugins',
            recommendation: 'Enable auto-updates for at least the high-trust plugins',
            rationale: 'CVEs in popular plugins (Elementor, Yoast, WPForms) are exploited within hours of disclosure. ' +
                'Auto-update closes the window.',
        });
    }
    // Warnings from wp_audit itself
    const auditWarnings = (meta['warnings'] ?? []);
    ...
    items.push({
        ...
        rationale: "Stolen admin credentials are the #1 WP compromise vector. 2FA defeats the entire class.",
    });
    items.push({
        priority: 'medium',
        category: 'plugins',
        recommendation: 'Install a security plugin with login attempt limiting',
        rationale: 'Wordfence / Sucuri / iThemes Security cap login attempts and block IPs — stops brute-force at the door.',
    });
    // Sort by priority
...
function toMarkdown(items, scanId) {
    const out = [];
...
//# sourceMappingURL=wpRecommendHardening.js.map