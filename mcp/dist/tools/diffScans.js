...
async function handler(input, ctx) {
    ...
    if (fromId.value === toId.value) {
        return failDomain('unknown_scan_id', `Cannot diff a scan against itself (${toId.value}).`);
    }
...
function resolveTo(inp, ctx) {
    ...
    if (!latest)
        return { ok: false, err: failDomain('unknown_scan_id', 'No completed scans yet.') };
...
function resolveFrom(inp, toScanId, ctx) {
    ...
    const previous = history.find((s) => s.scan_type === toScan.scan_type &&
        s.status === 'completed' &&
        s.scan_id !== toScanId &&
    ...
    if (!previous)
        return {
            ok: false,
            err: failDomain('unknown_scan_id', `No previous '${toScan.scan_type}' scan exists before ${toScanId}.`),
        };
