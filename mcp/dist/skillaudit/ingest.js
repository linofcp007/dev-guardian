...
export async function ingestTarget(targetRaw) {
    const target = targetRaw.trim();
    if (!target)
        return { ok: false, code: 'target_not_found', message: 'Empty target.' };
    // Remote: git or URL.
    if (/^git@/.test(target) || /\.git($|\?)/.test(target)) {
        return ingestGit(target);
    }
    if (/^https?:\/\//i.test(target)) {
        if (looksLikeGitHost(target))
            return ingestGit(target);
