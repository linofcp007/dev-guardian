...
export function buildSnapshot(storage, projectPath, now) {
    ...
    const cveItems = currentScan ? storage.cves.listActive(currentScan.scan_id) : [];
...
function toScanSummary(scan, now) {
...
function buildCoverage(currentScan) {
    const missingTools = currentScan?.missing_tools ?? [];
    const toolsRun = (currentScan?.tools_run ?? []).map((t) => t.name);
    const level = currentScan === null ? 'none' : missingTools.length > 0 ? 'partial' : 'full';
    return {
        level,
        tools_run: toolsRun,
...
function omittedCategoriesFor(missingTools) {
    const seen = new Set();
    const result = [];
...
function buildCveSummary(cveItems) {
    return {
...
function buildSincePrevious(storage, projectPath, currentScan, openFindings, suppressedFingerprints, truncation) {
...
function resolveBaseline(storage, projectPath) {
    ...
    if (scan === null || scan.project_path !== projectPath)
        return null;
    const resolved = { id: active.id, scan_id: active.scan_id, set_at: active.set_at };
    if (active.note !== undefined)
        resolved.note = active.note;
    return resolved;
}
function buildBaselineState(resolved, now) {
    if (resolved === null)
        return { active: null, age_days: null };
    const active = {
    ...
    if (resolved.note !== undefined)
        active.note = resolved.note;
    return {
        active,
        age_days: Math.floor((now - Date.parse(resolved.set_at)) / DAY_MS),
    };
}
function buildSuppressionState(activeSuppressions, now) {
