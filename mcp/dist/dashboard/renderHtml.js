...
function coverageBanner(coverage) {
    ...
    if (fullyMissing.length > 0) {
        sentences.push(`${fullyMissing.map((t) => escapeHtml(t)).join(', ')} did not run this scan — ` +
            `${categories} findings are NOT in these numbers.`);
    }
    if (partial.length > 0) {
        sentences.push(`${partial.map((t) => escapeHtml(t)).join(', ')} ran with reduced coverage — ` +
            `some ${categories} findings may be missing.`);
    }
    const message = sentences.length > 0 ? sentences.join(' ') : `${categories} findings are NOT in these numbers.`;
