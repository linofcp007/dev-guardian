...
function subcategoryFor(ruleId) {
    ...
    if (xss.has(ruleId))
        return 'xss';
    if (crypto.has(ruleId))
        return 'weak-crypto';
    if (cookie.has(ruleId))
        return 'cookie-misconfig';
    if (xxe.has(ruleId))
        return 'xxe';
    if (path.has(ruleId))
        return 'path-traversal';
    if (open_redirect.has(ruleId))
        return 'open-redirect';
    if (ldap.has(ruleId))
        return 'ldap-injection';
    return 'dotnet-security';
}
//# sourceMappingURL=securityCodeScan.js.map