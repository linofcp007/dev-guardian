...
```ts
import { describe, expect, it } from 'vitest';
import { analyzeRoutes, type AnalyzeInput } from '../../../src/dast/analyze.js';
...
function result(
  over: Partial<ProbeResult> & { request?: Partial<ProbeRequest> } = {},
): ProbeResult {
  const request: ProbeRequest = {
    id: 'anonymous GET /users', method: 'GET', path: '/users',
    url: 'http://localhost:3000/users', headers: {}, variant: 'anonymous',
    synthetic_params: false, route_index: 0, ...over.request,
  };
  const { request: _ignored, ...rest } = over;
  return {
...
describe('dast fingerprints', () => {
  it('is stable across a status change for the same check and route', () => {
    const a = analyzeRoutes(input({
      results: [result({ status: 302, headers: { location: 'https://evil.example.com/' } })],
    }));
    const b = analyzeRoutes(input({
      results: [result({ status: 307, headers: { location: 'https://evil.example.com/x' } })],
    }));
...
// reuse the same `result` / `input` helpers as analyze.route.test.ts —
// copy them into this file rather than importing across test files.
...
```ts
const checks = new Set(result.findings.map((f) => f.subcategory));
